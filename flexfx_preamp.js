...
function _initialize( tag )
{
    for( var preset = 1; preset <= 9; ++preset ) {
        $(tag+"_presetA"+preset).onclick = _on_select;
        $(tag+"_presetB"+preset).onclick = _on_select;
        for( var param = 1; param <= 5; ++param ) {
            for( var stage = 1; stage <= 3; ++stage ) {
                $(tag+"_rangeA"+preset+""+param+""+stage).oninput = _on_input; }}
    }
    return _on_property_received;
}

function _on_select( event )
{
...
function _on_input( event )
{
    ...
    //flexfx_send_property( _midi_port, property );
}
